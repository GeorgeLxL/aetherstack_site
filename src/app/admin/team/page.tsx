        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-6 py-3 text-left text-sm font-semibold text-primary">Avatar</th>
                ...
                <th className="px-6 py-3 text-left text-sm font-semibold text-primary">Skills</th>
              ...
              {members.map((member) => (
                <tr key={member.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-1">
                      {member.skills?.map((skill, i) => (
                        <span key={i} className="px-2 py-1 bg-gray-100 text-xs rounded-full">{skill}</span>
                      ))}
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md p-6 bg-white rounded-xl">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-primary">{editingId ? 'Edit' : 'Add'} Team Member</h2>
              <button onClick={() => setShowModal(false)}><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-primary mb-1">Avatar</label>
                <div className="flex items-center gap-4">
                  {formData.avatar_url ? (
                    <img src={formData.avatar_url} alt="Avatar" className="w-16 h-16 rounded-full object-cover" />
                  ) : (
              ...
              <div>
                <label className="block text-sm font-medium text-primary mb-1">Skills (comma-separated)</label>
                <input
                  type="text"
                  value={formData.skills}
                  onChange={e => setFormData({...formData, skills: e.target.value})}
              ...
              <div>
                <label className="block text-sm font-medium text-primary mb-1">Icon</label>
                <select
                  value={formData.icon}
                  onChange={e => setFormData({...formData, icon: e.target.value})}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
