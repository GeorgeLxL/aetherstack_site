...
export default function ClientEvaluations() {
  ...
  return (
    <section id="evaluations" className="py-20 bg-white dark:bg-gray-900">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-4">Client Evaluations</h2>
        ...
        {loading ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {displayEvaluations.map((evalItem) => (
              <div
                key={evalItem.id}
                className="bg-gray-50 dark:bg-gray-800 p-6 rounded-2xl border border-gray-100 dark:border-gray-700 hover:border-gray-200 dark:hover:border-gray-600 hover:-translate-y-1 hover:shadow-xl transition-all duration-300"
              >
                <div className="flex items-center gap-1 mb-4">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`w-5 h-5 ${i < evalItem.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300 dark:text-gray-600'}`}
                    />
                  ))}
                </div>
                <Quote className="w-8 h-8 text-gray-300 dark:text-gray-600 mb-3" />
                <p className="text-gray-600 dark:text-gray-300 mb-4 leading-relaxed">&quot;{evalItem.feedback}&quot;</p>
                <div className="border-t border-gray-100 dark:border-gray-700 pt-4 flex items-center gap-3">
                  {evalItem.avatar_url ? (
                    ...
                    <div>
                      <p className="font-semibold text-gray-900 dark:text-white">{evalItem.client_name}</p>
                      <p className="text-sm text-gray-600 dark:text-gray-300">{evalItem.company}</p>
                    </div>
                    </>
                  ) : (
                    <>
